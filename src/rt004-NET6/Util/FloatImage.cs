...
/// </summary>
public partial class FloatImage
{
  public const float RED_WEIGHT   = 0.2989f;
  public const float GREEN_WEIGHT = 0.5866f;
  public const float BLUE_WEIGHT  = 0.1144f;
    ...
    unsafe
    {
      ...
      fixed (float* poi = ni.data)
      {
        ...
        if (channels >= 3)
          ...
          {
            val = pi[0] * RED_WEIGHT + pi[1] * GREEN_WEIGHT + pi[2] * BLUE_WEIGHT;
            if (inv) val = 1.0 - val;
            if (gamma > 0.0)
  ...
  public bool GetPixel(int x, int y, float[] pix)
  {
    ...
    Buffer.BlockCopy(data, sizeof(float) * (origin + x * channels + y * stride), pix, 0, sizeof(float) * channels);
    return true;
  }
  ...
  public float GetGray(int x, int y)
  {
    ...
    return channels >= 3 ? data[i] * RED_WEIGHT + data[i + 1] * GREEN_WEIGHT + data[i + 2] * BLUE_WEIGHT : data[i];
  }
  ...
  public unsafe byte[]? Exposure(byte[]? result, double exp, double gamma = 0.0)
  {
    if (channels < 3)
      return null;

    int ostride = channels * width;

    if (result == null ||
        result.Length < stride * height)
      result = new byte[stride * height];
    ...
    fixed (float* id = data)
    {
      ...
      for (int y = 0; y < height; y++)
      {
        ...
        optr = ob + y * ostride;
        if (g > 0.0)
          for (int x = 0; x++ < width; iptr += channels, optr += channels)
          {
            optr[0] = (byte)Math.Clamp(255.999 * Math.Pow(iptr[0] * exp, g), 0.0, 255.0);
            optr[1] = (byte)Math.Clamp(255.999 * Math.Pow(iptr[1] * exp, g), 0.0, 255.0);
            optr[2] = (byte)Math.Clamp(255.999 * Math.Pow(iptr[2] * exp, g), 0.0, 255.0);
          }
        else
          for (int x = 0; x++ < width; iptr += channels, optr += channels)
          {
            optr[0] = (byte)Math.Clamp(255.999 * iptr[0] * exp, 0.0, 255.0);
            optr[1] = (byte)Math.Clamp(255.999 * iptr[1] * exp, 0.0, 255.0);
            optr[2] = (byte)Math.Clamp(255.999 * iptr[2] * exp, 0.0, 255.0);
          }
      }
    }
...
public class RadianceHDRFormat
{
  ...
  {
    if (startRgbe + 4 > rgbe.Length ||
        startRgb + 3 > rgb.Length)
      return;
    ...
    {
      double f = Math.Pow(2.0, rgbe[startRgbe + 3] - 136);
  ...
  public class HDRInstance
  {
    ...
    protected void oldReadScanline(int start, int len, Stream iss, int unget)
    {
      ...
      {
        _ = iss.Read(scanline, start, readBunch);
        start += readBunch;
    ...
    public FloatImage commonLoad(Stream stream)
    {
      ...
      _ = int.TryParse(readToken(stream), out int dim1);
      string axis2 = readToken(stream);
      _ = int.TryParse(readToken(stream), out int dim2);
      ...
      else if (char.ToLower(axis1[1]) == 'y')
      {
        height = dim1;
        bottomUp = axis1[0] == '+';
      }
      ...
      else if (char.ToLower(axis2[1]) == 'y')
      {
        height = dim2;
        bottomUp = axis2[0] == '+';
      }
    ...
    public void commonSave(Stream stream, FloatImage im)
    {
      ...
      if (!stream.CanWrite)
        throw new IOException("Cannot write to a Stream");
      ...
      _ = sb.Append(height).Append(" +X ").Append(width).Append('\n');
      ...
      for (y = 0; y < height; y++) // encode one scanline
      {
        ...
        {
          im.GetPixel(x, y, pix);
          RGBToRGBe(scanline, i, pix[0], pix[1], pix[2]);
        }
...
public partial class FloatImage
{
  ...
  public static FloatImage? FromFile(string fileName)
  {
    try
    {
      ...
      {
        RadianceHDRFormat.HDRInstance hi = new RadianceHDRFormat.HDRInstance();
  ...
  public void SavePFM(string fileName)
  {
    ...
    try
    {
      ...
      const int pixelSize = 3 * sizeof(float);
      var pixelBytes = new byte[pixelSize];
      float[] test = new float[1] { 1.0f };   // 0x3f800000
      Buffer.BlockCopy(test, 0, pixelBytes, 0, sizeof(float));
      string endian = pixelBytes[0] == 0 ? "-1.0" : "1.0";
      ...
      {
        int left = Scan0 + Stride * y * sizeof(float);
        for (int x = 0; x < Width; x++)
        {
          int i = left + Channels * x * sizeof(float);
          Buffer.BlockCopy(Data, i, pixelBytes, 0, pixelSize);
          fs.Write(pixelBytes, 0, pixelSize);
        }
      }
    }
    catch (IOException)
